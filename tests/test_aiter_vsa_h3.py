...
def _capturing(caplog, logger):
    """Capture records from an xfuser logger, which does not propagate to root.

    ``init_logger`` ends with ``logger.propagate = False`` (xfuser/logger.py), and caplog installs
    its handler on the root logger -- so nothing xfuser logs ever reaches it. Attaching the handler
    to the logger under test is what makes the assertion below read the logger this module actually
    warns through, rather than silently observing an empty list.
    """
...
def test_a_padded_tiling_runs_and_says_what_it_costs(recipe, caplog):
    ...
    with _capturing(caplog, vsa_h3_aiter.logger):
