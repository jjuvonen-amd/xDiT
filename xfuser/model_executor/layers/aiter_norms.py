"""AITER-accelerated RMSNorm utilities for models that use diffusers' Python RMSNorm.

``diffusers.models.normalization.RMSNorm`` is a Python implementation that up-casts
to float32; AITER's CK-Tile ``rms_norm`` kernel operates natively in bfloat16.

These utilities are model-agnostic — any model that uses diffusers RMSNorm for its
block normalizations can benefit from ``_replace_rms_norms_with_aiter``.
"""
